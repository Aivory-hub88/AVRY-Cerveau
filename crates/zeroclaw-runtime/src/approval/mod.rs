...
impl ApprovalManager {
    ...
    pub fn derive_for_risk_profile(&self, risk_profile: &RiskProfileConfig) -> Self {
        Self {
            ...
            session_allowlist: Mutex::new(HashSet::new()),
            audit_log: Mutex::new(Vec::new()),
            // Cerveau (enterprise-hardening round 1): a derived manager
            // keeps the parent's taxonomy/ledger/pending wiring. Dropping
            // them here would silently strip F-2 idempotency and the
            // Irreversible-tier Pending floor from delegated executions —
            // REVIEW: confirm inherit (not fresh None) is intended.
