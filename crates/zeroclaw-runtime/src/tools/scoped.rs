            ...
            if let Some(registry) = shared_registry {
                ...
                if config.mcp.deferred_loading {
                    ...
                    if allowed_stub_count > 0 && !list_deferred_mcp_specs {
                        ...
                        // Cerveau (Phase 4.2): attach the capability graph when
                        // both a tenant-scoped turn and the process-wide ranker
                        // are present. Absent either (vanilla install, or
                        // `[capability_graph].enabled = false`, the default),
                        // `tool_search` behaves exactly as it did before this
                        // feature existed.
                        ...
                        if let (Some(t), Some(ranker)) = (
                            ...
                            tool_search =
                                tool_search.with_capability_graph(t.tenant_id.clone(), ranker);
                        }
