...
impl DelegateTool {
    ...
    async fn execute_sync_with_admission(
        ...
        let (result, fallback) = zeroclaw_providers::reliable::scope_provider_fallback(async {
            let result = self
                .execute_sync_in_target_overlay(agent_name, prompt, args, admission)
                .await;
    ...
    /// Runs the sync sub-turn under the target specialist's tenant overlay
    /// (no-op outside a tenant turn). Kept apart from the provider-fallback
    /// wrapper above so the two scopes compose without sharing a body.
    async fn execute_sync_in_target_overlay(
        &self,
        agent_name: &str,
        prompt: &str,
        args: &serde_json::Value,
...
impl DelegateTool {
    ...
    async fn execute_agentic_with_admission(
        ...
        let sub_approval = crate::approval::current_delegation_approval()
            .and_then(|parent| {
